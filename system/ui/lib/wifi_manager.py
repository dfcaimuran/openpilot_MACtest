...
# On non-Linux platforms (e.g. macOS), WifiManager cannot talk to system D-Bus / NetworkManager.
# Provide a no-op implementation so the UI can run without crashing.
if platform.system() != "Linux":
  cloudlog.warning("WifiManager: using dummy implementation on non-Linux platform")

  class _DummyWifiManager:
    def __init__(self):
    ...
    # Callback registration (ignored on macOS, but keep API)
    def add_callbacks(self, need_auth: Callable[[str], None] | None = None,
                      activated: Callable[[], None] | None = None,
                      forgotten: Callable[[], None] | None = None,
                      networks_updated: Callable[[list[Network]], None] | None = None,
                      disconnected: Callable[[], None] | None = None):
      # Do nothing; we never call these in dummy mode
      return

    def process_callbacks(self):
      # No-op: nothing queued in dummy mode
      self._callback_queue = []

    def set_active(self, active: bool):
      # Just store the flag; no background scanning on macOS
      self._active = active

    # Connection management no-ops
    def connect_to_network(self, ssid: str, password: str, hidden: bool = False):
      cloudlog.warning(f"WifiManager dummy: ignoring connect_to_network('{ssid}', ...) on non-Linux platform")

    def forget_connection(self, ssid: str, block: bool = False):
      cloudlog.warning(f"WifiManager dummy: ignoring forget_connection('{ssid}') on non-Linux platform")

    def activate_connection(self, ssid: str, block: bool = False):
      cloudlog.warning(f"WifiManager dummy: ignoring activate_connection('{ssid}') on non-Linux platform")

    def _deactivate_connection(self, ssid: str):
      cloudlog.warning(f"WifiManager dummy: ignoring _deactivate_connection('{ssid}') on non-Linux platform")
    ...
    def set_ipv4_forward(self, enabled: bool):
      # Not supported on macOS in this dummy implementation
      self._ipv4_forward = enabled

    def set_tethering_active(self, active: bool):
      cloudlog.warning("WifiManager dummy: tethering not supported on non-Linux platform")

    def set_current_network_metered(self, metered: MeteredType):
      self._current_network_metered = metered

    def update_gsm_settings(self, roaming: bool, apn: str, metered: bool):
      cloudlog.warning("WifiManager dummy: GSM settings not supported on non-Linux platform")

    def stop(self):
      self._exit = True

  # Expose dummy implementation under the original name
  WifiManager = _DummyWifiManager
